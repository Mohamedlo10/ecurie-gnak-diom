...
function Login() {
	...
	const NonValide =
		(currentStep == 0 && (user.nom == "" || user.prenom == "")) ||
		(currentStep == 1 &&
			(user.email == "" ||
				user.motdepasse == "" ||
				user.confirmPassword == "" ||
				EmailValidity(user.email) ||
				user.motdepasse != user.confirmPassword));
                  ...
                  Forgot your password?
						...
						{!isLogin && (
							<div className="grid gap-2">
								...
								{user.motdepasse != user.confirmPassword && (
									<span className="text-red-700">Mot de passe different</span>
								)}
							</div>
						)}
					</>
			...
			{isLogin || currentStep == 2 ? (
				...
				<div className="flex flex-row gap-8">
					<button
						...
						onClick={backStep}
						className={`w-full h-13   mt-2 bg-zinc-100 text-black  rounded-md ${
							currentStep == 0
								? "opacity-50"
