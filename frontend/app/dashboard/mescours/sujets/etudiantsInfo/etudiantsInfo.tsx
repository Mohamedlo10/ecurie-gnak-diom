...
import { insererEtudiantDansCours, supprimerEtudiantInCours } from "@/app/api/suivre/query";
...
const deleteEtudiant = async (idutilisateur:string) => {
  if(coursId)
  {
    try{
      const response = await supprimerEtudiantInCours(coursId,idutilisateur)
        ...
        <div className="p-4 mt-24 flex w-[30vw]">
            { !isAddingEtudiant ? (
             ...
             <div className="flex w-full max-w-xl flex-col items-center justify-center bg-slate-50 p-8 text-left">
                       ...
                       <div className="text-base grid grid-cols-1 gap-auto items-center justify-center font-bold w-full mt-4">
                         ...
                         {user?.role=="professeur" &&(<div className="w-full flex justify-end items-end px-10">
                            <Button
                                      ...
                                      disabled={user?.role!="professeur"}
                                      className="w-fit h-10 cursor-pointer text-sm font-bold bg-red-700"
                                    onClick={() => deleteEtudiant(selectedEtudiant.idutilisateur)}
                                    >
              ...
              <h2 className="mb-8 text-2xl font-bold">Ajouter un Nouveau etudiant</h2>
              ...
              <input
                  ...
                  placeholder="Mail de l'etudiant"
                  className="border p-2 rounded w-full"
                  required
                />
            </div>
            <div className="ml-auto pt-8 w-full items-center justify-center flex font-medium">
                  <Button type="submit"  className="w-fit h-10 font-bold">Ajouter l'etudiant</Button>
                </div>
              </form>
            </div>
            )}
